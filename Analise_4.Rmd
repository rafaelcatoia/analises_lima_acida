...
fit_lm <- lm(data=dframe_rhiz_ufscar %>% filter(Safra=='2021'),
          formula = Rhizotron~RegHidrico+Tratamento+RegHidrico*Tratamento)

fit_aov <- aov(data=dframe_rhiz_ufscar %>% filter(Safra=='2021'),
          formula = Rhizotron~RegHidrico+Tratamento+RegHidrico*Tratamento)
...
##### 2022 
```{r,fig.align='center',fig.height=8,fig.width=8,dpi=200}
fit_lm <- lm(data=dframe_rhiz_ufscar %>% filter(Safra=='2022'),
          formula = Rhizotron~RegHidrico+Tratamento+RegHidrico*Tratamento)

fit_aov <- aov(data=dframe_rhiz_ufscar %>% filter(Safra=='2022'),
          formula = Rhizotron~RegHidrico+Tratamento+RegHidrico*Tratamento)
